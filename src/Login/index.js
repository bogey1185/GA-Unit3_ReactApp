...
class Login extends Component {
  ...
  handleSubmit = async (event) => {
    ...
    try {
      ...
      const loginResponse = await fetch(`${process.env.REACT_APP_PATH}/api/v1/users/${loginPath}`, {
        method: 'POST',
      ...
      if (parsedLoginResponse.sysMsg === 'login successful'){
        ...
        if (this.state.userDesignation === 'landlord') {
          ...
          //read only
          const propResponse = await fetch(`${process.env.REACT_APP_PATH}/api/v1/properties/show/${this.state.propertyCode}`, {
            method: 'GET',
            // credentials: 'included',
            headers: {'Content-Type': 'application/json'}
          })
